...
export function generateThermalReceiptHTML(data: ReceiptData): string {
  const sym = data.currencySymbol ?? "DA";
  const fmt = (n: number) => `${n.toLocaleString("fr-DZ")} ${sym}`;
...
<body>
  ...
  ${data.storePhone ? `<div class="center">Tél: ${data.storePhone}</div>` : ""}
  <hr>
  <div>N° Facture: ${data.invoiceNumber}</div>
  <div>Date: ${data.date}</div>
  <div>Caissier: ${data.cashier}</div>
  ${data.customer ? `<div>Client: ${data.customer}</div>` : ""}
  <hr>
  <table>${itemsHtml}</table>
  <hr>
  <div style="display:flex;justify-content:space-between"><span>Sous-total</span><span>${fmt(data.subtotal)}</span></div>
  <div style="display:flex;justify-content:space-between"><span>Remise</span><span>-${fmt(data.discount)}</span></div>
  <div style="display:flex;justify-content:space-between"><span>TVA (19%)</span><span>${fmt(data.tax)}</span></div>
  ...
  ${paymentsHtml}
  <hr>
  <div class="center">${data.footer ?? "Merci pour votre visite ! À bientôt."}</div>
